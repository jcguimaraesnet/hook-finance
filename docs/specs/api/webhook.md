...
- **Body esperado (banco antigo):**
  ...
  ```
- **Body esperado (app novo — cartão 2236):**
  ```json
  {
    "title": "Mercado Livre",
  ...
  ```
  Padrão diferente: `title` carrega a descrição da despesa direto; `text` só tem o valor (sem data/hora/cartão). Parser detecta pelo match de `Pagou R$` no `text` e hardcoda `cardLast4 = "2236"`. Data/hora viram o instante do POST. Ver [../rules/webhook-parser.md](../rules/webhook-parser.md).
...
- **Inserção:**
  ...
  - col H (Cartão): `cardLast4` extraído (banco antigo) ou `"2236"` (app novo).
